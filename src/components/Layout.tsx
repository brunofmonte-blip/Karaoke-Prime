...
const BasicLobby = () => {
  ...
  // Efeito para garantir que a câmera toque assim que o stream for capturado
  useEffect(() => {
    if (cameraActive && webcamRef.current && webcamRef.current.srcObject) {
      webcamRef.current.play().catch(e => console.log("Aguardando interação para play da câmera"));
    }
  }, [cameraActive]);
  ...
  const startCamera = async () => {
    try {
      ...
      alert("ERRO: O acesso à câmera foi bloqueado. Verifique as permissões no seu navegador (ícone de cadeado).");
    }
  };
  ...
  const finalizeSession = () => {
    // 💡 AJUSTE AQUI: Mude para 'true' se quiser simular um teste com nota positiva
    ...
    if (!userCantoReal) {
      score = "0.0";
      note = "O Rigor da IA Prime não detectou entrada de áudio suficiente. A análise espectral foi interrompida por falta de dados vocais.";
  ...
  return (
    <div className="min-h-screen bg-black relative overflow-x-hidden">
      ...
      {/* 1. INTERFACE DO LOBBY (OCULTA QUANDO O VÍDEO ESTÁ ATIVO) */}
      {!selectedVideo && (
        <div className="relative z-10 p-4 md:p-8 max-w-6xl mx-auto pt-20 animate-in fade-in duration-700 text-center">
          ...
          <div className="max-w-4xl mx-auto relative mb-16">
            ...
            <Input placeholder="Qual hit vamos cantar hoje?" value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={handleSearch} className="pl-16 h-20 bg-black/80 border-primary/30 text-white rounded-3xl focus:border-primary text-xl font-bold shadow-2xl" />
          </div>

          {results.length > 0 && (
            <div className="max-w-4xl mx-auto space-y-6 pb-20">
              {results.map((video) => (
                <div key={video.id.videoId} onClick={() => setSelectedVideo(video.id.videoId)} className="bg-black/60 border border-white/5 rounded-[2.5rem] p-5 flex items-center gap-8 hover:bg-white/5 cursor-pointer group hover:border-primary/50 transition-all">
      ...
      {/* 2. SALA DE PERFORMANCE (VÍDEO E WEBCAM) */}
        ...
        <div key={videoKey} className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center p-4 animate-in fade-in duration-1000">
          <div className="flex flex-col lg:flex-row items-center gap-12 w-full max-w-7xl">
            ...
            <div className="flex-1 w-full aspect-video rounded-[3rem] overflow-hidden border-2 border-primary/30 bg-zinc-950 relative shadow-[0_0_80px_rgba(0,168,225,0.1)]">
              {/* O Player nativo permite pausar clicando no vídeo */}
            ...
            <div className="flex flex-col items-center gap-8">
              ...
              <div className="flex flex-col gap-6 w-full">
                <Button onClick={finalizeSession} className="h-20 px-10 rounded-full bg-primary text-black font-black text-2xl italic hover:bg-white transition-all shadow-[0_0_30px_rgba(0,168,225,0.5)] uppercase tracking-tighter">FINALIZAR SHOW</Button>
