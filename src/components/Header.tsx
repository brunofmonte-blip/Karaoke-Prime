...
import { Mic2, ShieldCheck, LogOut, User as LucideUser } from 'lucide-react';
        ...
        <nav className="hidden md:flex gap-6">
          {navItems.map(item => (
            <span 
              ...
              className={`text-[10px] font-black uppercase tracking-widest cursor-pointer transition-colors ${location.pathname === item.path ? 'text-primary' : 'text-gray-400 hover:text-white'}`}
            >
              {item.name}
            </span>
          ))}
                ...
                <div className="flex items-center gap-2 bg-white/5 border border-white/10 px-3 py-1.5 rounded-full">
                  {user.photoURL ? (
                    ...
                    <div className="w-6 h-6 rounded-full bg-primary/20 flex items-center justify-center border border-primary/50">
                      <LucideUser size={12} className="text-primary" />
                    </div>
                  )}
