...
import { Mic2, LogOut, User as LucideUser } from 'lucide-react';
        ...
        <div className="flex items-center gap-4">
          {user ? (
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 bg-white/5 border border-white/10 px-3 py-1.5 rounded-full shadow-lg">
                {user.photoURL ? (
                  <img src={user.photoURL} alt="Avatar" className="w-6 h-6 rounded-full border border-primary/50 object-cover" />
                ) : (
                  <div className="w-6 h-6 rounded-full bg-primary/20 flex items-center justify-center border border-primary/50"><LucideUser size={12} className="text-primary" /></div>
                )}
                <span className="text-white text-[10px] font-black tracking-widest uppercase hidden sm:block">{user.displayName?.split(' ')[0]}</span>
              </div>
              <Button onClick={handleLogout} variant="ghost" size="icon" title="Sair" className="text-gray-500 hover:text-white hover:bg-destructive/20 rounded-full h-8 w-8 transition-colors">
                <LogOut size={16} />
              </Button>
            </div>
          ) : (
