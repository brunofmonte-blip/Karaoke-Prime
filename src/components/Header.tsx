  ...
  return (
    <div className="min-h-screen bg-black flex flex-col pt-16 font-sans text-white overflow-x-hidden">
      ...
      <section className="max-w-7xl mx-auto px-4 w-full mb-32">
        <h2 className="text-5xl font-black text-white text-center mb-16 italic tracking-tighter uppercase">Mural de <span className="text-orange-500 neon-gold-glow">Conquistas</span></h2>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
          {[ {icon: Star, t: 'Early Bird', c: 'text-primary'}, {icon: Mic2, t: 'Rock Star', c: 'text-orange-500'}, {icon: Trophy, t: 'Duel Master', c: 'text-primary'}, {icon: Medal, t: 'Perfect Pitch', c: 'text-orange-500'}, {icon: Globe, t: 'Polyglot', c: 'text-primary'}].map((b, i) => (
            <Card key={i} className="bg-zinc-950 border-white/10 flex flex-col items-center p-8 text-center rounded-[2rem] hover:border-white/30 transition-all">
              <div className={`h-20 w-20 rounded-full border-2 flex items-center justify-center mb-6 bg-black ${b.c} border-current shadow-[0_0_15px_rgba(251,146,60,0.2)]`}><b.icon className="h-10 w-10" /></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[ {n:'Brasil', s:'BR', f:'br'}, {n:'EUA', s:'US', f:'us'}, {n:'Japão', s:'JP', f:'jp'}, {n:'Reino Unido', s:'GB', f:'gb'}].map((c, i) => (
            <Card key={i} className="bg-zinc-950 border-white/10 p-8 rounded-[2rem] hover:border-primary transition-colors flex flex-col items-center">
              ...
              <p className="text-gray-500 text-[10px] font-black uppercase mb-6">{c.n}</p>
              <Button variant="outline" className="rounded-full text-[10px] font-black uppercase tracking-widest h-8 border-white/20 hover:bg-white hover:text-black">Explorar</Button>
            </Card>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 w-full mb-32">
        <h2 className="text-3xl font-black text-white mb-8 italic uppercase tracking-tighter">Trend Topics <span className="text-orange-500">🔥</span></h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {[1,2,3,4].map(i => (
            <div key={i} className="relative aspect-[9/16] rounded-3xl overflow-hidden group cursor-pointer border border-white/10 bg-zinc-900 shadow-2xl">
              <img src={`https://picsum.photos/seed/sing${i}/600/800`} className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:scale-110 transition-transform duration-700" alt="Singer" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent p-6 flex flex-col justify-end">
                <p className="font-black text-white text-sm italic">@VocalStar_{i}</p>
                <p className="text-[10px] text-primary font-bold uppercase tracking-widest mt-1">🎵 Rolling in the Deep</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            {[1,2,3,4].map(i => (
              <Card key={i} className="bg-zinc-950 border-white/5 p-4 rounded-2xl flex items-center gap-4 hover:bg-white/5 transition-colors cursor-pointer">
                <div className="h-16 w-16 bg-zinc-800 rounded-lg shrink-0 overflow-hidden border border-white/10"><img src={`https://picsum.photos/seed/cover${i}/150/150`} alt="Cover" className="w-full h-full object-cover" /></div>
                <div className="truncate"><p className="font-black text-white text-sm truncate uppercase italic">Música {i}</p><p className="text-[10px] text-gray-500 uppercase font-black tracking-widest">Artista X</p></div>
              </Card>
            ))}
          </div>
        </div>
      ...
      {/* 7. RANKINGS GLOBAIS (ANEXO 7) */}
      <section className="max-w-7xl mx-auto px-4 w-full mb-32">
         <h2 className="text-5xl font-black text-primary text-center mb-16 uppercase italic tracking-tighter drop-shadow-[0_0_15px_rgba(0,168,225,0.4)]">Rankings Globais</h2>
         <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {['Global Top 5', 'Nacional Top 3', 'Local Top 2'].map((title, idx) => (
              <Card key={title} className={`bg-zinc-950 p-8 rounded-[2.5rem] border-2 shadow-2xl ${idx === 0 ? 'border-primary shadow-primary/5' : idx === 1 ? 'border-orange-500 shadow-orange-500/5' : 'border-white/10'}`}>
                <h3 className={`text-xl font-black text-center mb-8 uppercase italic ${idx === 0 ? 'text-primary' : idx === 1 ? 'text-orange-500' : 'text-white'}`}>{title}</h3>
                <div className="space-y-5">
                  {[1,2,3,4,5].slice(0, idx === 0 ? 5 : idx === 1 ? 3 : 2).map(pos => (
                    <div key={pos} className="flex justify-between items-center border-b border-white/5 pb-3 last:border-0">
                      <div className="flex items-center gap-3"><span className="text-gray-600 font-black text-sm w-4">{pos}</span><div className="h-8 w-8 rounded-full bg-zinc-800 border border-white/10" /><span className="text-white text-xs font-bold uppercase truncate max-w-[100px]">User_Prime_{pos}</span></div>
      ...
      {/* 8. DASHBOARD DINÂMICO (ANEXO 1 ATUAL) */}
      <section className="max-w-7xl mx-auto px-4 w-full mb-32 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <div className="text-center space-y-6 bg-zinc-950 border border-white/10 p-12 rounded-[2.5rem] shadow-neon-blue">
            <h3 className="text-3xl font-black text-white italic">{user ? `Bem-vindo, ${user.displayName?.split(' ')[0]}!` : "Pronto para Cantar?"}</h3>
            <p className="text-gray-400 font-medium">{user ? "Seus dados de evolução estão sendo processados pela nossa IA." : "Cante sua primeira música para gerar os gráficos!"}</p>
            <p className="text-orange-500 font-black uppercase tracking-[0.3em] text-[10px] pt-6 flex items-center justify-center gap-2"><Sparkles size={14} /> IA PRONTA PARA ANALISAR</p>
          </div>
        </div>
        <Card className="bg-zinc-950 border-white/10 rounded-[2.5rem] shadow-2xl p-10 overflow-hidden relative">
          <div className="flex items-center gap-6 mb-10 pb-10 border-b border-white/10">
            ...
            <div>
              ...
              <p className={`text-[10px] font-black tracking-widest uppercase inline-block px-3 py-1 rounded-md ${user ? "text-primary bg-primary/10" : "text-orange-500 bg-orange-500/10"}`}>{user ? "Prime Member" : "Guest Access"}</p>
            </div>
          </div>
          <div className="space-y-4">
              <div className="flex justify-between py-2 border-b border-white/5"><span className="text-[10px] font-black uppercase text-gray-500 tracking-widest flex items-center gap-2"><Star size={12} className="text-orange-500" /> Melhor Nota</span><span className="font-black text-white text-lg">{user ? "94.8%" : "0.0%"}</span></div>
              <div className="flex justify-between py-2 border-b border-white/5"><span className="text-[10px] font-black uppercase text-gray-500 tracking-widest flex items-center gap-2"><GraduationCap size={12} className="text-orange-500" /> Nível Academy</span><span className="font-black text-white text-lg">{user ? "Lvl 1" : "0"}</span></div>
              <div className="flex justify-between py-2"><span className="text-[10px] font-black uppercase text-gray-500 tracking-widest flex items-center gap-2"><Trophy size={12} className="text-orange-500" /> Ranking Global</span><span className="font-black text-white text-lg">{user ? "#412" : "9999"}</span></div>
          </div>
        </Card>
      </section>
