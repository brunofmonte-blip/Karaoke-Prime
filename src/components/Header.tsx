...
import { Mic2, ShieldCheck, LogOut } from 'lucide-react';
...
const Header = () => {
  ...
  // 💡 Lendo a memória do navegador para saber se mostra "Login" ou "Sair"
  const isLoggedIn = localStorage.getItem('@KaraokePrime:loggedIn') === 'true';

  const handleLogout = () => {
    localStorage.removeItem('@KaraokePrime:loggedIn');
    window.location.href = '/'; // Recarrega e joga pra Home
  };
    ...
    <header className="fixed top-0 w-full z-50 bg-black/80 backdrop-blur-md border-b border-white/10">
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          {isLoggedIn ? (
            <Button onClick={handleLogout} variant="outline" className="border-destructive/50 text-destructive hover:bg-destructive hover:text-white font-bold text-xs uppercase tracking-widest h-9 px-6 rounded-full transition-all">
