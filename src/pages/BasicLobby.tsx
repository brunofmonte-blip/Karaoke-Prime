...
const BasicLobby = () => {
  ...
  // 🔴 ADICIONE SUA CHAVE API NESTA LINHA ABAIXO:
  ...
  return (
    <div className="min-h-screen bg-black relative overflow-x-hidden font-sans">
      ...
      <div className="relative z-10 p-4 md:p-8 max-w-6xl mx-auto pt-20 animate-in fade-in duration-700">
        <button onClick={() => navigate('/')} className="text-gray-400 hover:text-white mb-6 flex items-center gap-2 font-bold uppercase tracking-widest text-[10px] transition-colors">
        ...
        {selectedVideo && (
          <div className="fixed inset-0 z-[100] bg-black/95 flex flex-col items-center justify-center p-4 backdrop-blur-md animate-in zoom-in duration-300">
            <button 
              onClick={() => setSelectedVideo(null)} 
              className="absolute top-8 right-8 text-white hover:text-primary transition-colors flex items-center gap-2 font-black tracking-tighter text-lg italic border-b-2 border-primary/30 hover:border-primary p-2 z-[110]"
            >
               FINALIZAR SHOW <X size={24} />
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          ...
          <div onClick={() => navigate('/duel')} className="group cursor-pointer p-8 rounded-[2rem] border border-white/10 bg-black/60 hover:border-destructive/50 hover:bg-destructive/10 transition-all flex flex-col items-center text-center shadow-xl">
            <div className="h-16 w-16 rounded-2xl bg-white/5 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform group-hover:bg-destructive/20">
              <Users className="h-10 w-10 text-white group-hover:text-destructive" />
            </div>
        ...
        <div className="max-w-4xl mx-auto relative mb-16 group">
          ...
          <Input 
            ...
            className="pl-16 h-20 bg-black/80 border-primary/30 text-white rounded-3xl focus:border-primary text-xl font-bold shadow-2xl transition-all" 
          />
        </div>
            ...
            <div className="space-y-6">
              {results.map((video) => (
                <div 
                  ...
                  className="bg-black/60 border border-white/5 rounded-[2.5rem] p-5 flex items-center gap-8 hover:bg-white/5 transition-all cursor-pointer group hover:border-primary/50 shadow-xl"
                >
                  <div className="relative w-56 h-32 overflow-hidden rounded-2xl border border-white/10">
                    ...
                    <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                       <PlayCircle className="text-primary h-14 w-14 drop-shadow-lg" />
                    </div>
                  </div>
                  ...
                  <PlayCircle className="text-primary h-12 w-12 mr-6 hidden md:block opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
              ))}
            </div>
          </div>
