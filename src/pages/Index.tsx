...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col pt-16 font-sans">
      ...
      {/* 1. HERO SECTION (Bateria ao fundo e Texto Correto) */}
      <section className="relative min-h-[90vh] flex flex-col items-center justify-center p-4">
        {/* Usando uma imagem de estúdio musical/bateria para bater com o original */}
        <div className="absolute inset-0 bg-cover bg-center opacity-30" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1514320291840-2e0a9bf2a9ae?q=80&w=2000')" }} />
        ...
        {/* 5 Produtos Principais (Alinhamento Canto Superior Esquerdo) */}
      ...
      <section className="relative z-20 max-w-5xl mx-auto w-full px-4 -mt-8 mb-24">
        <div className="flex bg-black/40 border border-white/20 rounded-full p-2 items-center backdrop-blur-md">
      ...
      <section className="max-w-7xl mx-auto px-4 w-full mb-32">
        <h2 className="text-4xl font-black text-orange-500 text-center mb-8 drop-shadow-[0_0_15px_rgba(249,115,22,0.6)]">Conquistas (Badges)</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-6 opacity-60">
          {[{icon: Star, t: 'Early Bird', d: 'Completed your very first vocal session.'}, {icon: Mic2, t: 'Rock Star', d: 'Completed 5 songs in the Rock Classics genre.'}, {icon: Trophy, t: 'Duel Master', d: 'Won 10 local duels against the AI opponent.'}, {icon: Medal, t: 'Perfectionist', d: 'Achieved a perfect 100% pitch accuracy score.'}, {icon: Globe, t: 'Polyglot', d: 'Sang songs in two different languages.'}].map((b, i) => (
            <Card key={i} className="bg-black/40 border-white/5 flex flex-col items-center p-6 text-center hover:border-white/20 transition-all cursor-pointer">
              <div className="h-16 w-16 rounded-full border border-gray-600 flex items-center justify-center mb-6"><b.icon className="h-8 w-8 text-gray-400" /></div>
              <h4 className="font-bold text-white mb-3">{b.t}</h4>
      ...
      {/* 4. MÚSICAS POR PAÍS (Bandeiras Retangulares e Siglas) */}
      <section className="max-w-7xl mx-auto px-4 w-full mb-32">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[
            ...
            <Card key={i} className="bg-black/40 border-white/10 p-8 flex flex-col items-center text-center hover:border-primary/50 transition-colors">
              <img src={c.f} alt={c.n} className="w-20 h-14 object-cover rounded-sm mb-6 shadow-xl border border-white/10" />
      ...
      {/* 5. TREND TOPICS (SINGERS) - Formato Vertical Estilo Reels */}
      <section className="max-w-7xl mx-auto px-4 w-full mb-32">
            ...
            <div key={i} className="relative aspect-[9/16] rounded-2xl overflow-hidden group cursor-pointer border border-white/10">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent p-4 flex flex-col justify-end">
                <div className="flex items-center gap-2 mb-2">
                   <div className="h-6 w-6 rounded-full bg-primary/20 border border-primary/50" />
      ...
      {/* 7. RANKINGS GLOBAIS (Com Avatares e Bordas Coloridas) */}
      <section className="max-w-7xl mx-auto px-4 w-full mb-32">
        <h2 className="text-4xl font-black text-primary text-center mb-12 drop-shadow-[0_0_15px_rgba(0,168,225,0.5)]">Rankings Globais</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="bg-black/60 border-primary shadow-[0_0_20px_rgba(0,168,225,0.2)] p-6 rounded-2xl">
            ...
            <div className="space-y-4">
              {[ {n:'VocalPro_88', s:'98.5'}, {n:'SingStar_JP', s:'97.1'}, {n:'OperaQueen', s:'96.9'}, {n:'MikeDrop', s:'95.8'}, {n:'MelodyX', s:'95.5'} ].map((u, i) => (<div key={i} className="flex justify-between items-center py-2 border-b border-white/5 last:border-0"><div className="flex gap-4 items-center"><span className="text-gray-500 font-bold text-sm w-4">{i+1}</span><div className="h-8 w-8 rounded-full bg-primary/20 border border-primary/30" /><span className="text-white font-bold text-sm">{u.n}</span></div><span className="text-orange-500 font-black">{u.s}</span></div>))}
            </div>
          </Card>
          <Card className="bg-black/60 border-orange-500 shadow-[0_0_20px_rgba(251,146,60,0.2)] p-6 rounded-2xl">
            ...
            <div className="space-y-4">
              {[ {n:'LocalHero_NY', s:'95.2'}, {n:'TexasTenor', s:'94.8'}, {n:'CaliSinger', s:'94.1'} ].map((u, i) => (<div key={i} className="flex justify-between items-center py-2 border-b border-white/5 last:border-0"><div className="flex gap-4 items-center"><span className="text-gray-500 font-bold text-sm w-4">{i+1}</span><div className="h-8 w-8 rounded-full bg-orange-500/20 border border-orange-500/30" /><span className="text-white font-bold text-sm">{u.n}</span></div><span className="text-orange-500 font-black">{u.s}</span></div>))}
            </div>
          </Card>
          <Card className="bg-black/60 border-white/20 p-6 rounded-2xl">
            ...
            <div className="space-y-4">
              {[ {n:'DowntownDiva', s:'93.5'}, {n:'BaritoneBob', s:'92.9'} ].map((u, i) => (<div key={i} className="flex justify-between items-center py-2 border-b border-white/5 last:border-0"><div className="flex gap-4 items-center"><span className="text-gray-500 font-bold text-sm w-4">{i+1}</span><div className="h-8 w-8 rounded-full bg-white/10 border border-white/30" /><span className="text-white font-bold text-sm">{u.n}</span></div><span className="text-orange-500 font-black">{u.s}</span></div>))}
            </div>
          </Card>
        </div>
      </section>
