...
import { Search, Mic2, GraduationCap, Star, Lock, Music, LayoutDashboard, Sparkles, Trophy, Globe, Medal, Heart, MessageCircle, ArrowRight, User as LucideUser } from "lucide-react";
...
<p className="text-[10px] text-gray-400 mt-2 font-bold uppercase">Karaoke tradicional e batalhas e duetos online.</p>
</div>
</Card>
...
<p className="text-[10px] text-gray-400 mt-2 font-bold uppercase">10 níveis com treinamento profissional.</p>
</div>
</Card>
{[
{ n: "Next Talent", d: "Audições gamificadas pelo mundo.", i: Globe, path: "/talent" },
{ n: "Backstage", d: "Dashboard avançado bloqueado.", i: LayoutDashboard, path: "/backstage" },
{ n: "Next Success", d: "Transforme ideias em músicas IA.", i: Sparkles, path: "/next-success" }
].map((item) => (
<Card key={item.n} onClick={() => navigate(item.path)} className="cursor-pointer opacity-50 hover:opacity-100 hover:border-white/30 transition-all p-6 rounded-3xl border border-white/10 bg-black/60 h-56 relative flex flex-col justify-between">
<Lock className="absolute top-6 right-6 text-gray-500 h-4 w-4" />
<div className="h-12 w-12 rounded-xl bg-white/5 flex items-center justify-center text-gray-500"><item.i size={24} /></div>
<div>
<h3 className="font-black text-gray-400 text-lg uppercase italic">{item.n}</h3>
<p className="text-[10px] text-gray-500 mt-2 font-bold uppercase">{item.d}</p>
</div>
</Card>
))}
</div>
