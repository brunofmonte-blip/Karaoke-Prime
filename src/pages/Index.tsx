...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col pt-16 font-sans text-white overflow-x-hidden">
      ...
      {/* 3. MURAL DE CONQUISTAS (ANEXO 4) */}
      <section className="max-w-7xl mx-auto px-4 w-full mb-32">
        <h2 className="text-5xl font-black text-white text-center mb-16 italic tracking-tighter uppercase drop-shadow-lg">Mural de <span className="text-orange-500 neon-gold-glow">Conquistas</span></h2>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
          {[ {icon: Star, t: 'Early Bird', c: 'text-primary'}, {icon: Mic2, t: 'Rock Star', c: 'text-orange-500'}, {icon: Trophy, t: 'Duel Master', c: 'text-primary'}, {icon: Medal, t: 'Perfect Pitch', c: 'text-orange-500'}, {icon: Globe, t: 'Polyglot', c: 'text-primary'}].map((b, i) => (
            <Card key={i} className="bg-zinc-950 border-white/10 flex flex-col items-center p-8 text-center rounded-[2rem] hover:border-white/30 transition-all">
              <div className={`h-20 w-20 rounded-full border-2 flex items-center justify-center mb-6 bg-black ${b.c} border-current`}><b.icon className="h-10 w-10" /></div>
      ...
      {/* 4. MÚSICAS POR PAÍS (ANEXO 4 ABAIXO) */}
      <section className="max-w-7xl mx-auto px-4 w-full mb-32 text-center">
        <h2 className="text-4xl font-black text-white mb-12 flex items-center justify-center gap-4 uppercase italic tracking-tighter"><Globe className="text-primary h-10 w-10" /> Músicas Mais Cantadas</h2>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[ {n:'Brasil', s:'BR', f:'br'}, {n:'EUA', s:'US', f:'us'}, {n:'Japão', s:'JP', f:'jp'}, {n:'Reino Unido', s:'GB', f:'gb'}].map((c, i) => (
            <Card key={i} className="bg-zinc-950 border-white/10 p-8 rounded-[2rem] hover:border-primary transition-colors flex flex-col items-center">
              <img src={`https://flagcdn.com/w160/${c.f}.png`} className="w-16 mb-4 rounded shadow-lg" alt={c.n} />
              <h2 className="text-4xl font-black text-white mb-1 italic">{c.s}</h2>
              <p className="text-gray-500 text-xs font-bold uppercase mb-6">{c.n}</p>
              <Button variant="outline" className="rounded-full text-[10px] font-black uppercase tracking-widest">Explorar</Button>
            </Card>
          ))}
        </div>
      </section>

      {/* 5. TREND TOPICS SINGERS (ANEXO 5) */}
      <section className="max-w-7xl mx-auto px-4 w-full mb-32">
        <h2 className="text-4xl font-black text-white mb-8 italic uppercase tracking-tighter">Trend Topics <span className="text-orange-500">🔥</span></h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {[1,2,3,4].map(i => (
            <div key={i} className="relative aspect-[9/16] rounded-3xl overflow-hidden group cursor-pointer border border-white/10 bg-zinc-900">
              <img src={`https://picsum.photos/seed/singer${i}/600/800`} className="absolute inset-0 w-full h-full object-cover opacity-60" alt="Singer" />
              <div className="absolute inset-0 bg-gradient-to-t from-black p-6 flex flex-col justify-end">
                <p className="font-black text-white text-sm italic">@Talento_{i}</p>
                <p className="text-[10px] text-primary font-bold uppercase">🎵 Rolling in the Deep</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* 6. RECENTLY ADDED E TAGS (ANEXO 6) */}
      <section className="max-w-7xl mx-auto px-4 w-full mb-32 grid grid-cols-1 lg:grid-cols-2 gap-16">
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {[1,2,3,4].map(i => (
              <Card key={i} className="bg-zinc-950 border-white/5 p-4 rounded-2xl flex items-center gap-4">
                <div className="h-16 w-16 bg-zinc-800 rounded-lg shrink-0 overflow-hidden"><img src={`https://picsum.photos/seed/song${i}/150/150`} alt="Cover" /></div>
                <div className="truncate"><p className="font-black text-white text-sm truncate">Música Top {i}</p><p className="text-[10px] text-gray-500 uppercase font-black">Artista Nome</p></div>
              </Card>
            ))}
          </div>
        </div>
        <div>
          <h2 className="text-3xl font-black text-white mb-8 italic uppercase tracking-tighter flex gap-3">Trend Tags <span className="text-primary">#</span></h2>
          <div className="space-y-3">
             {['Sertanejo2026', 'RockClassics', 'VocalAcademy'].map(tag => (
               <div key={tag} className="bg-zinc-950 border border-white/10 rounded-2xl p-4 flex justify-between items-center hover:border-primary cursor-pointer transition-all">
                 <p className="font-black text-white text-sm italic">#{tag}</p>
                 <span className="text-green-400 text-xs font-black">+22% ↗</span>
               </div>
             ))}
          </div>
        </div>
      ...
      {/* 7. RANKINGS (ANEXO 7) */}
      <section className="max-w-7xl mx-auto px-4 w-full mb-32">
         <h2 className="text-5xl font-black text-primary text-center mb-16 uppercase italic tracking-tighter drop-shadow-neon">Rankings Globais</h2>
         <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {['Global Top 5', 'Nacional Top 3', 'Local Top 2'].map((title, idx) => (
              <Card key={title} className={`bg-zinc-950 p-8 rounded-[2rem] border-2 ${idx === 0 ? 'border-primary' : idx === 1 ? 'border-orange-500' : 'border-white/10'}`}>
                <h3 className="text-xl font-black text-center mb-8 uppercase italic">{title}</h3>
                <div className="space-y-4">
                  {[1,2,3].map(pos => (
                    <div key={pos} className="flex justify-between items-center border-b border-white/5 pb-2">
                      <span className="text-gray-500 font-black">{pos}</span>
                      <span className="text-white text-sm font-bold truncate mx-4">Cantor_Alpha_{pos}</span>
                      <span className="text-primary font-black">98.5</span>
                    </div>
                  ))}
                </div>
              </Card>
      ...
      {/* 8. DASHBOARD DINÂMICO (ANEXO 8 - SEU ANTIGO ANEXO 2) */}
        ...
        <div>
          ...
          <div className="text-center space-y-6 bg-zinc-950 border border-white/10 p-12 rounded-[2.5rem]">
            ...
            <p className="text-gray-400 font-medium">
              {user ? "Seus dados de evolução estão sendo processados pela nossa IA. Continue praticando!" : "Sua jornada vocal começa aqui. Cante sua primeira música para gerar os gráficos!"}
            </p>
          </div>
        </div>
        <Card className="bg-zinc-950 border-white/10 rounded-[2.5rem] shadow-2xl p-10 overflow-hidden">
          <div className="flex items-center gap-6 mb-10 pb-10 border-b border-white/10">
            {user?.photoURL ? (
              ...
              <div className="h-24 w-24 rounded-full border-2 border-primary/50 bg-primary/10 flex items-center justify-center shadow-lg"><UserIcon size={40} className="text-primary" /></div>
            )}
            <div>
          ...
          <div className="space-y-6">
              <div className="flex justify-between py-3 border-b border-white/5"><span className="text-[10px] font-black uppercase text-gray-500 tracking-widest">Melhor Nota</span><span className="font-black text-white text-xl">{user ? "94.8%" : "0.0%"}</span></div>
              <div className="flex justify-between py-3 border-b border-white/5"><span className="text-[10px] font-black uppercase text-gray-500 tracking-widest">Nível Academy</span><span className="font-black text-white text-xl">{user ? "Lvl 1" : "0"}</span></div>
          </div>
        </Card>
      </section>
      ...
      <div className="text-center pb-24 pt-16 border-t border-white/10 bg-zinc-950">
        <h2 className="text-5xl md:text-8xl font-black text-white tracking-tighter italic uppercase mb-10 opacity-20">CANTE. EVOLUA. CONQUISTE.</h2>
